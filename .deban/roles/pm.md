...
- [ ] The relaxation appendix orders quad corners **clockwise about the centroid**, but stage 4 "normalize winding to CCW". If the relaxation target-square formula assumes CW order, post-CCW data may relax wrong (grid blows up or fails to square). Must verify empirically; may need to feed relaxation CW-ordered corners regardless of stored winding. — owner: minikai — since: 2026-05-27
- [ ] Acceptance gate asserts "dual cells tile the interior with no gaps/overlaps", but angle-sorted centroid polygons are only guaranteed simple for well-behaved vertices. Highly irregular/near-degenerate vertices could yield self-intersecting dual cells. Untested until M2. — owner: minikai — since: 2026-05-27
